...
export async function DELETE(
  ...
  try {
    ...
    // Delete token using use case
    const useCase = new DeleteAccessTokenUseCase(tokenRepository, buildRepository);
    ...
    // If deletion was prevented due to usage, return 409 Conflict
    if (!result.deleted && result.buildsUsingToken) {
      return NextResponse.json(
        {
          error: 'Token is in use by builds',
          buildsUsingToken: result.buildsUsingToken,
        },
        { status: 409 }
      );
    }
