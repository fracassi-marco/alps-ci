...
## 4. Build Card Enhancements

#### 4.1. Display Test Statistics ✅ COMPLETED
```
Prompt: Display the number of tests in the last run and the number of failed tests for each build. Check if the uploaded artifacts contain files matching the `*test*.xml` pattern. Extract test data using the JUnit format. If no data is found, hide the test section in the build card.

- Added TestStats interface to domain models (totalTests, failedTests, passedTests, skippedTests)
- Created junit-parser utility with parseJUnitXML and isTestArtifact functions
- Added fetchArtifacts and downloadArtifact methods to GitHubGraphQLClient
- Added artifact fetching support to CachedGitHubClient
- Updated FetchBuildStatsUseCase to fetch test statistics from most recent workflow run artifacts
- Regex-based JUnit XML parser supports multiple formats:
  - Single <testsuite> element
  - <testsuites> wrapper with aggregated stats
  - Multiple <testsuite> elements (sums all)
- Attribute order-independent parsing
- BuildCard displays test statistics in purple-themed section (only shown when data exists)
- BuildListView displays test statistics in expanded row details (only shown when data exists)
- Test statistics hidden when no artifacts found or parsing fails 
```
---
